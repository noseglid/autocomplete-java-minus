...
class Provider {
  ...
  setRegistry(registry) {
    ...
    this.finders = {
      plain: new PlainEntryFinder(this.registry),
      dot: new DotEntryFinder(this.registry),
      constructor: new ConstructorEntryFinder(this.registry)
    };
  }
  ...
  getSuggestions({ editor, bufferPosition, scopeDescriptor, prefix, activatedManually }) {
    if (!this.registry || !prefix || prefix.length === 0) {
      return [];
    }
