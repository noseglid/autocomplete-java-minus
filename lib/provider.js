...
class Provider {
  ...
  getSuggestions({ editor, bufferPosition, scopeDescriptor, prefix, activatedManually }) {
    ...
    if (dotPrefix) {
      /**
       * Dot prefixed, e.g. `variable.prefix`.
       * This should show members of the variable.
       */
      return this.getStaticClassDotPrefixedSuggestions(editor, dotPrefix, prefix)
        .sort(this.sortByNameKey)
        .slice(0, atom.config.get('autocomplete-plus.maxVisibleSuggestions'))
        .map(entry => this.mapMethodEntryToSuggestion(entry));
    } else if (hasNewPrefix) {
      /**
       * Prefix is immediately preceded by the word `new`, eg. `new Prefix`
       * This should return public constructors.
       */
      return [].concat.apply([], this.getPublicConstructorClassSuggestions(prefix)
        .sort(this.sortByNameKey)
        .slice(0, atom.config.get('autocomplete-plus.maxVisibleSuggestions'))
        .map(entry => this.mapClassEntryToConstructorSuggestion(entry))
      );
    }

    /* The default case where a class is plainly typed out */
    return this.getClassSuggestions(prefix)
      .sort(this.sortByNameKey)
      .slice(0, atom.config.get('autocomplete-plus.maxVisibleSuggestions'))
      .map(entry => this.mapClassEntryToSuggestion(entry));
  }
