...
class DotEntryFinder extends EntryFinder {
  ...
  reduceDotChain(editor, bufferPosition) {
    return (state, entry, index, array) => {
      ...
      if (!state.fullyQualifiedClass) {
        ...
        return this.guessType(editor, entry, bufferPosition);
      }
      ...
      if (state.static) {
        ...
        const nestedClass = this.registry.get(`${state.fullyQualifiedClass}$${entry}`);
        if (nestedClass) {
          return {
      ...
      const klass = this.registry.get(state.fullyQualifiedClass);
      if (!klass) {
  ...
  getNestedClasses(fullyQualifiedClass, prefix) {
    ...
    const nestedClassSuggestions = this.registry.filter(nestedClassSearch);
  ...
  get(editor, bufferPosition, prefix) {
    ...
    const dotChainEndContext = dotChain.reduce(this.reduceDotChain(editor, bufferPosition), {});
