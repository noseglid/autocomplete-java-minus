...
function methodEntryToSnippet(entry) {
  ...
  const mapArg = (arg, index) => `\${${index + 1}:${denamespace(arg).replace(/\$/, '.')}}`;
...
function mapToSuggestion(entry) {
  switch (entry._type) {
    ...
    case 'constructor': return mapClassEntryToConstructorSuggestion(entry);
    default:
...
export {
  mapToSuggestion,
  mapClassEntryToConstructorSuggestion,
  mapClassEntryToSuggestion,
  mapMethodEntryToSuggestion,
  mapFieldEntryToSuggestion
};
